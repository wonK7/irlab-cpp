...
def pick_up(options, robot):
    ...
    while True:
        ...
        while not holding_bottle:
            # Capture an image and run ML on it.
            X, image, vision_tform_dogtoy = get_obj_and_img(
                network_compute_client, _ml_service, _model,
                confidence, kImageSources, 'med')
            
            hand_frame = image.shot.hand_color_image_sensor  # Should be "hand_color_image_sensor"
            vision_tform_hand = frame_helpers.get_a_tform_b(
                image.shot.transforms_snapshot,
                frame_helpers.VISION_FRAME_NAME,
                hand_frame)
            hand_tform_obj = vision_tform_hand.inverse() * vision_tform_dogtoy

            robot_state = robot_state_client.get_robot_state()
            robot_tform_hand = frame_helpers.get_a_tform_b(
                robot_state.kinematic_state.transforms_snapshot,
                frame_helpers.GRAV_ALIGNED_BODY_FRAME_NAME,  # Or use ODOM_FRAME_NAME
                hand_frame)
            robot_tform_obj = robot_tform_hand * hand_tform_obj

            print("Object position relative to robot base:")
            print(f"x: {robot_tform_obj.x:.2f}, y: {robot_tform_obj.y:.2f}, z: {robot_tform_obj.z:.2f}")
            ...
            # Side grasp (gripper x-axis aligned with +X in vision frame)
